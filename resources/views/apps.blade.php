...
<script>
  ...
  try {
  const data = await fetchWithAuth(`${BASE_URL || '{{ env("APP_URL") }}'}/api/data-object/categories/${id}/contents`);
  renderContents(data);
  } catch (error) {
  ...
  Promise.all([
  fetchWithAuth(`${BASE_URL || '{{ env("APP_URL") }}'}/api/data-object/categories`),
  fetchWithAuth(`${BASE_URL || '{{ env("APP_URL") }}'}/api/data-object/task-codes`)
  ]).then(([cats, tsk]) => {
  categories = cats;
  tasks = tsk;
