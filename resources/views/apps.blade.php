...
<head>
  ...
  <style>
    /* (style sama seperti kode Anda, tidak diubah) */
    * {
      margin: 0;
      padding: 0;
    ...
    body {
      ...
      background: var(--tg-theme-bg-color, #ffffff); color: var(--tg-theme-text-color, #000000); padding: 16px; }
      .container-custom { max-width: 600px; margin: 0 auto; }
      .nav-tabs { display: flex; gap: 16px; border-bottom: 1px solid var(--tg-theme-hint-color, #e5e5ea); margin-bottom: 20px; }
      .nav-tabs button { background: none; border: none; padding: 10px 0; font-size: 16px; cursor: pointer; color: var(--tg-theme-hint-color, #8e8e93); }
      .nav-tabs button.active { color: var(--tg-theme-link-color, #007aff); border-bottom: 2px solid var(--tg-theme-link-color, #007aff); }
      .search-box { width: 100%; padding: 8px 12px; border-radius: 20px; border: 1px solid var(--tg-theme-hint-color, #e5e5ea); background: var(--tg-theme-secondary-bg-color, #e9e9ef); color: var(--tg-theme-text-color, #000); margin-bottom: 16px; }
      .category-item, .content-item, .task-item { background: var(--tg-theme-secondary-bg-color, #f5f5f5); border-radius: 12px; padding: 12px 16px; margin-bottom: 8px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; }
      .category-name { font-weight: 600; }
      .category-code { font-size: 12px; color: var(--tg-theme-hint-color); }
      .content-code, .task-code { font-family: monospace; background: var(--tg-theme-bg-color, #fff); padding: 4px 8px; border-radius: 8px; }
      .back-link { cursor: pointer; color: var(--tg-theme-link-color, #007aff); margin-bottom: 12px; display: inline-block; }
      .loading { text-align: center; padding: 40px; color: var(--tg-theme-hint-color); }
      .badge { background: var(--tg-theme-link-color, #007aff); color: white; padding: 4px 8px; border-radius: 20px; font-size: 12px; }
      ...
      if (searchTerm) filtered = categories.filter(c => c.name.toLowerCase().includes(searchTerm) || c.code.toLowerCase().includes(searchTerm));
      ...
      html += `<div class="category-item" data-id="${cat.id}" data-code="${escapeHtml(cat.code)}">
      <div><div class="category-name">${escapeHtml(cat.name)}</div><div class="category-code">${escapeHtml(cat.code)}</div></div>
      ...
      async function showContents(categoryId) {
      ...
      html += `<div class="content-item" data-code="${escapeHtml(item.code)}">
      <div><div class="content-desc">${escapeHtml(item.description)}</div><div class="content-code">${escapeHtml(item.code)}</div></div>
      ...
      let filtered = tasks;
      if (searchTerm) filtered = tasks.filter(t => t.code.toLowerCase().includes(searchTerm) || t.description.toLowerCase().includes(searchTerm));
      ...
      html += `<div class="task-item" data-code="${escapeHtml(task.code)}">
      <div><div class="task-desc">${escapeHtml(task.description)}</div><div class="task-code">${escapeHtml(task.code)}</div></div>
      <i class="bi bi-copy"></i>
      </div>`;
      });
      tasksContainer.innerHTML = html;
      document.querySelectorAll('.task-item').forEach(el => {
      el.addEventListener('click', () => copyToClipboard(el.dataset.code));
      });
      }
      ...
      // Panggil startApp dengan token yang sudah tersedia
      startApp(token);
      </script>
      </body>
      </html>